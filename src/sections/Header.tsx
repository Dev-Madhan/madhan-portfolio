                    ...
                    {navItems.map(({ label, href }) => (
                        <a href={href} key={label} className='text-stone-200 border-t last:border-b border-stone-800 py-8 group/nav-item relative isolate' onClick={handleClickMobileNavItem}>
                            <div className='container !max-w-full flex items-center justify-between'>
                                <span className='text-3xl group-hover/nav-item:pl-4
                                transition-all duration-500'>{label}</span>
                            ...
                            <div className='absolute w-full h-0 bg-gray-green-800 group-hover/nav-item:h-full transition-all duration-500 bottom-0 -z-10'></div>
                        </a>
                    ))}
                </nav>
            </div>
